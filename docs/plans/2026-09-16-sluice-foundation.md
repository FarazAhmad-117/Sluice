...
- **The org revocation public key must be pinned in customer-controlled configuration.** `verifyRevocation` takes the key from its caller. If the SDK fetches that key from the Sluice server or from Convex at startup, the whole threat model collapses: a compromised server serves its own public key, signs its own notices, and kills every customer fleet. The key belongs in an env var, a config file, or baked in at install, and it must never arrive over the same channel that delivers the notice. Key rotation needs a designed story at the same time, because pinning makes rotation painful by construction.
- **Epochs must be globally monotonic per token id and must never reset.** Verification proves authenticity, not freshness, so the SDK refuses any epoch at or below the highest it has seen. That defence only holds if epochs never restart on token rotation and token ids are never reused. The worst case is a fresh process with no persisted epoch floor: it has seen nothing, so any captured genuine notice for that token id is above its floor and replaying it is a working denial of service. Either persist the floor across restarts or bind a freshness window to `revokedAt`.
- **Do not build replay defence on signature-byte deduplication.** Ed25519 admits multiple valid encodings of a signature for one message unless the verifier enforces canonical `S`, and noble's default verify is permissive. Nobody can forge a notice, but an attacker can produce different bytes that verify for the same notice. The epoch counter is immune to this and is the correct defence.
- **The SDK must sanitise `reason` before printing it.** The module validates but deliberately does not sanitise, because sanitising would diverge from the signed bytes. A reason containing ANSI escapes can clear an operator's screen or render a clickable link, `\r` can overwrite the line so the real reason never appears, and U+202E reverses visual order so a shutdown log reads as something other than its bytes. Strip C0 and C1 controls and bidi overrides at the render site. The shutdown log is the one place an operator looks during an incident.
- **Tightening `assertValidNotice` later is a silent breaking change.** The same validator runs on both sign and verify, so tightening it retroactively invalidates every notice already signed and stored, and the failure mode is `false`, meaning "do not shut down". Any tightening ships as `sluice/revocation/v2` with both verifiers live, following the `AUTH_INFO` and `UNWRAP_INFO` versioning convention in `token.ts`.
- **A notice carries no `orgId`.** Fine while an SDK pins exactly one org key. If an SDK ever trusts several org keys at once, a notice signed by org A becomes valid against a token id belonging to org B, since token ids are not namespaced in the signed bytes. Add `orgId` to the encoding under `/v2` before multi-org trust becomes real.
- **`revokedAt` is signed but never checked against a clock.** It is display and audit only, and must not be turned into a freshness signal without the replay work above, because an attacker replaying an old notice replays its `revokedAt` unchanged.
